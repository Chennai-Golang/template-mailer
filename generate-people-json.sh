...
jq -s '[ .[0] + .[1] | group_by(.Email | ascii_downcase)[] | add ]' $_invite_file_path $_reminder_file_path | jq '[ sort_by(.Email | ascii_downcase) | .[] | .IsRSVPed = false ]' > mails/existing.json

_file1_path="mails/existing.json"
_file2_path="mails/current-rsvp.json"

# Sort files before comparing - DESTRUCTIVE!
cat $_file1_path | jq 'sort_by(.Email | ascii_downcase)' > "$_file1_path-sorted"
mv "$_file1_path-sorted" $_file1_path

cat $_file2_path | jq 'sort_by(.Email | ascii_downcase)' > "$_file2_path-sorted"
mv "$_file2_path-sorted" $_file2_path

# Check for duplicates
jq -r '.[] | .Email | ascii_downcase' $_file1_path > /tmp/uniq-mail-check-file1.txt
jq -r '.[] | .Email | ascii_downcase' $_file2_path > /tmp/uniq-mail-check-file2.txt
